...
fun ICorChainDsl<AppContext>.repoDelete(title: String) = worker {
    ...
    handle {
        val request = DbChatterIdRequest(chatterRepoPrepare)
        when(val result = chatterRepo.deleteChatter(request)) {
            is DbChatterResponseOk -> chatterRepoDone = result.data
            is DbChatterResponseErr -> {
                fail(result.errors)
                chatterRepoDone = chatterRepoRead
            }
